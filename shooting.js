...
class Raven{
    constructor(){
        ...
        this.x = canvas.width;
        this.y = Math.random() * (canvas.height - 50 - this.height) + 50;
    ...
    update(deltatime){
        if(this.y < 55 || this.y > canvas.height - this.height){
...
function drawScore(){
    ...
    ctx.fillStyle = 'white';
    ctx.fillText('Score: ' + score, 55, 155);
}
...
function reloadGame(e) {
    ...
    if (clickX >= canvas.width/2 - 100 && clickX <= canvas.width/2 + 100 &&
        clickY >= canvas.height/2 + 50 && clickY <= canvas.height/2 + 100) {
        // Reload the page
...
function animate(timestamp){
    ...
    drawScore();
    [...particle, ...ravens, ...explosion].forEach(object => object.update(deltatime));
    [...particle, ...ravens, ...explosion].forEach(object => object.draw());
    ravens = ravens.filter(object => !object.markedForDeletion);
    explosion = explosion.filter(object => !object.markedForDeletion);
    particle = particle.filter(object => !object.markedForDeletion);
...
document.addEventListener('mousemove', function(e) {
    var cursor = document.querySelector('.cursors');
